...
import { OrderToolContext } from "~/context/order-context";
...
import { useToolCart } from "~/hooks/useCart";
...
import { Button } from "~/components/main/button";
import { createToolsOrder } from "~/server/orders";
// import { RoastedBeansProductSchema } from "~/schema/product";
...
export const useOrderFormLoader = routeLoader$<InitialValues<CreateToolOrderForm>>(() => {
    return {
        nama_pembeli: '',
        nomor_whatsapp: '',
        alamat: '',
...
export const useOrderFormAction = formAction$<CreateToolOrderForm>(
    async (values, event) => {
        await createToolsOrder({ values, event });
        // console.info(values)
    },
    {
        validate: valiForm$(OrderToolSchema),
...
export default component$(() => {
    ...
    const [form, { Form, Field, FieldArray }] = useForm<CreateToolOrderForm>({
    ...
    const {
        ...
        decrementQuantity,
        removeItem
    } = useToolCart(order, form, selectedShipping, shippings);
